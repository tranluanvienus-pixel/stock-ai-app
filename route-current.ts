...
async function analyzeWithGroq(params: {
  ...
  verdict: string; verdictVi: string;
  rsi: number; stochRSI: number;
  ma20: number; ma50: number; ma200: number | null;
  macd: { macdLine: number; signal: number };
  bb: { upper: number; lower: number; middle: number };
  atr: number; adx: { adx: number };
  obv: { obvTrend: string };
  ivRank: number; perf5d: number; perf20d: number;
  reasons_buy: string[]; reasons_avoid: string[];
}): Promise<{ groqSummary: string; groqAdvice: string; groqRisk: string } | null> {
  if (!GROQ_API_KEY) return null;
  try {
    const prompt = `Bạn là chuyên gia phân tích chứng khoán Mỹ. Phân tích cổ phiếu ${params.symbol}:

GIÁ & ĐIỂM: $${params.price.toFixed(2)} | Điểm: ${params.score}/100 | ${params.verdictVi}
...
MACD: ${params.macd.macdLine.toFixed(3)} | BB: $${params.bb.lower.toFixed(2)}-$${params.bb.upper.toFixed(2)}
OBV: ${params.obv.obvTrend} | IV Rank: ${params.ivRank.toFixed(0)}% | ATR: ${params.atr.toFixed(2)}
Perf 5d: ${params.perf5d.toFixed(2)}% | 20d: ${params.perf20d.toFixed(2)}%
Lý do mua: ${params.reasons_buy.slice(0,3).join("; ")}
Lý do tránh: ${params.reasons_avoid.slice(0,3).join("; ")}

Trả lời JSON thuần túy (không markdown):
{"groqSummary":"tóm tắt xu hướng 2-3 câu tiếng Việt","groqAdvice":"lời khuyên hành động cụ thể 2-3 câu: nên mua/bán/chờ, giá vào, stop loss gợi ý","groqRisk":"rủi ro chính 1-2 câu ngắn"}`;
    ...
    const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "Chuyên gia chứng khoán. Chỉ trả lời JSON thuần túy." },
...
export async function POST(req: Request) {
  ...
  const groqResult = await analyzeWithGroq({
    symbol, price: currentPrice, score, verdict, verdictVi,
    rsi, stochRSI, ma20, ma50, ma200, macd, bb, atr, adx, obv,
