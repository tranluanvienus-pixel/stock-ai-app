...
// Fear & Greed Index — multiple fallback sources
async function getFearGreed() {
  // Try Alternative.me API (reliable, no CORS issues)
  try {
    ...
    if (res.ok) {
      ...
      if (item) {
        ...
        return { score, rating: item.value_classification };
      }
    }
  } catch {}
  // Try CNN as fallback
  ...
  // Estimate from VIX if both fail
  return null;
}
