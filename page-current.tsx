        ...
        {result && (
          <div className="space-y-3">
            ...
            {(result.groqSummary || result.groqAdvice || result.groqRisk) && (
              <div className="bg-gradient-to-r from-purple-950 to-blue-950 rounded-xl p-3 border border-purple-800">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-purple-400 text-base">🤖</span>
                  <div className="text-xs font-bold text-purple-300 uppercase tracking-wider">
                    {t("GROQ AI · PHÂN TÍCH TỔNG HỢP", "GROQ AI · COMPREHENSIVE ANALYSIS")}
                  </div>
                  <span className="text-xs bg-purple-800 text-purple-300 px-2 py-0.5 rounded-full ml-auto">Llama 3.3 70B</span>
                </div>
                {result.groqSummary && (
                  <div className="bg-purple-900/40 rounded-lg p-3 mb-2 border border-purple-800/50">
                    <div className="text-xs text-purple-400 font-medium mb-1">📊 {t("Tóm tắt thị trường", "Market Summary")}</div>
                    <div className="text-xs text-gray-200 leading-relaxed">{result.groqSummary}</div>
                  </div>
                )}
                {result.groqAdvice && (
                  <div className="bg-green-900/30 rounded-lg p-3 mb-2 border border-green-800/50">
                ...
                {result.groqRisk && (
                  <div className="bg-red-900/30 rounded-lg p-3 border border-red-800/50">
                    <div className="text-xs text-red-400 font-medium mb-1">⚠️ {t("Rủi ro cần lưu ý", "Key Risks")}</div>
