...
export async function POST(req: Request) {
  ...
  // After-hours — Yahoo meta postMarket/preMarket + Finnhub
  const postMarket = result.meta.postMarketPrice;
  const preMarket = result.meta.preMarketPrice;
  // Finnhub cung cấp giá pre/post market khi thị trường đóng
  const finnhubPre = fq?.pc && fq?.o && Math.abs(fq.o - fq.pc) > 0.01 ? fq.o : null;
  
  let afterHoursPrice: number | undefined = undefined;
  let afterHoursLabel = "";
  
  if (postMarket && Math.abs(postMarket - currentPrice) > 0.01) {
    afterHoursPrice = postMarket;
    afterHoursLabel = "Post-market";
  } else if (preMarket && Math.abs(preMarket - currentPrice) > 0.01) {
    afterHoursPrice = preMarket;
    afterHoursLabel = "Pre-market";
  } else if (finnhubPre && Math.abs(finnhubPre - currentPrice) > 0.01) {
    afterHoursPrice = finnhubPre;
    afterHoursLabel = "Pre-market";
  }
